<?php
    ...
    if(isset($_POST['update']))
    {
    	...
    	if(!empty($restaurant_name) || !empty($vat) || !empty($address) || !empty($phone) || !empty($country) || !empty($description) || !empty($currency))
    	{
    		if($count == 0)
    		{
    			mysqli_query($db, "INSERT INTO info(restaurant_name, vat, address, country, description, currency) VALUES('$restaurant_name', '$vat', '$address', '$country', '$description', '$currency')");
    			$message = "Data Inserted";
    		}
    		else
    		{
    			mysqli_query($db, "UPDATE info SET restaurant_name = '$restaurant_name', vat = '$vat', address = '$address', country = '$country', description = '$description', currency = '$currency' ");
